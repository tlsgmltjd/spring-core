...
public class OrderServiceImpl implements OrderService {
    ...
    // @Quilfier -> @Quilfier끼리 매칭 -> 빈 이름 매칭
    // - 빈에 붙은 @Quilfier와 주입할때 붙은 @Quilfier가 같은 빈을 주입시킨다. 못 찾았다면 @Quilfier의 이름의 빈을 추가로 찾는다.
    // @Primary 사용
    // - 주입하려는 타입의 여러 빈이 있다면 @Primary가 붙은 빈이 우선으로 주입됨
    // @Primary는 기본값처럼 동작, @Quilfier는 상세하게 동작한다. 넓은 범위보다 좁은 범위의 선택권이 우선순위가 더 높다. @Quilfier가 우선권이 더 높음
    @Autowired
